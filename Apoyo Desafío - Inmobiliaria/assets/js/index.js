...
function filtro() {
  ...
  if (
    roomInput == '' | metrosDesdeInput == '' | metrosHastaInput == ''
  ) {
  ...
  propiedadesJSON.forEach((propiedad) => {
    if (propiedad.rooms >= Number(roomInput) && propiedad.m >= Number(metrosDesdeInput) && propiedad.m <= Number(metrosHastaInput)) {
      propiedadesHtml += `
      ...
      `
      contadorPropiedades +=1;
    }
  });
  
  mansionesHtml += `<h4 class="py-3">Total: <span id="total">${contadorPropiedades}</span></h4>
  <div class="propiedades" id="propiedadesDiv">
        </div>
  `
